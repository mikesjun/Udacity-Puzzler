...
public class puzzlerGameLogic : MonoBehaviour {
	...
	void displayPattern() { //Invoked repeating.
		...
		if (currentlyDisplayingPattern == true) { //If we are not finished displaying the pattern
			if (currentDisplayIndex < puzzleOrder.Length) { //If we haven't reached the end of the puzzle
				...
				puzzleSpheres [puzzleOrder [currentDisplayIndex]].GetComponent<orbLightUp>().patternLightUp (puzzleSpeed); //Light up the sphere at the proper index.  For now we keep it lit up the same amount of time as our interval, but could adjust this to be less.
